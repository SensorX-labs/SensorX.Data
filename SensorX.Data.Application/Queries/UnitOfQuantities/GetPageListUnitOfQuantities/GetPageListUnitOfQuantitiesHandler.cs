...
public sealed class GetPageListUnitOfQuantitiesHandler(
    IQueryBuilder<UnitOfQuantity> unitQueryBuilder,
    IQueryExecutor queryExecutor
) : IRequestHandler<GetPageListUnitOfQuantitiesQuery, Result<OffsetPagedResult<GetPageListUnitOfQuantitiesResponse>>>
{
    public async Task<Result<OffsetPagedResult<GetPageListUnitOfQuantitiesResponse>>> Handle(
        GetPageListUnitOfQuantitiesQuery request,
        CancellationToken cancellationToken)
    {
        try
        {
            IQueryable<UnitOfQuantity> query = unitQueryBuilder.QueryAsNoTracking;
            ...
            var items = await queryExecutor.ToListAsync(
                query.OrderByDescending(x => x.CreatedAt)
                    ...
                    .Select(x => new GetPageListUnitOfQuantitiesResponse(
                        x.Id.Value,
                        x.Name,
            ...
            return Result<OffsetPagedResult<GetPageListUnitOfQuantitiesResponse>>.Success(
                new OffsetPagedResult<GetPageListUnitOfQuantitiesResponse>
                {
                    Items = items,
        ...
        {
            return Result<OffsetPagedResult<GetPageListUnitOfQuantitiesResponse>>.Failure(
                $"Lỗi khi lấy danh sách đơn vị tính: {ex.Message}");
        }
    }
}
